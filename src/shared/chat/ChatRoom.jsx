...
/**
 * Full chat room screen. `roomMeta` is the item from ChatInbox's list (has
 * the counterpart's name/logo/poster for the header) — optional, the room
...
export default function ChatRoom({ chatRoomId, roomMeta, onBack }) {
  ...
  const [yourRole, setYourRole] = useState(null); // LEAD | MEMBER | COMPANY
  ...
  const headerSub = roomMeta?.companies ? [roomMeta.companies.industry].filter(Boolean).join(" · ") : roomMeta?.projects ? [roomMeta.projects.project_type].filter(Boolean).join(" · ") : "";
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20, height: "100%", minHeight: 0 }}>
      <div style={{ display: "flex", flexDirection: "column", border: `1px solid ${C.border}`, borderRadius: 16, background: "#fff", overflow: "hidden", minHeight: 0 }}>
        {/* header */}
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", borderBottom: `1px solid ${C.border}` }}>
          {onBack && (
            <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "1.1rem", color: C.muted }}>←</button>
          )}
          {headerImg ? (
            <img src={headerImg} alt="" style={{ width: 36, height: 36, borderRadius: 9, objectFit: "cover", border: `1px solid ${C.border}` }} />
          ) : (
            <div style={{ width: 36, height: 36, borderRadius: 9, background: C.cream, border: `1px solid ${C.border}` }} />
          )}
          <div>
            <div style={{ fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: "0.92rem", color: C.navy }}>{headerName}</div>
            {headerSub && <div style={{ fontSize: "0.74rem", color: C.muted }}>{headerSub}</div>}
          </div>
          {roomMeta?.deal_status === "CLOSED" && (
            <span style={{ marginLeft: "auto", background: C.goldPale, color: "#7A5C25", fontSize: "0.68rem", fontWeight: 700, padding: "3px 10px", borderRadius: 100 }}>Deal Active</span>
          )}
        </div>

        {/* messages */}
        <div style={{ flex: 1, overflowY: "auto", padding: "18px", display: "flex", flexDirection: "column", gap: 10, minHeight: 0 }}>
          <ErrorBanner message={error} onRetry={load} />
          {!messages && !error && <div style={{ display: "flex", justifyContent: "center", padding: "40px 0" }}><Spinner size={24} /></div>}
          {messages && messages.length === 0 && (
            <div style={{ textAlign: "center", color: C.muted, fontSize: "0.85rem", padding: "40px 0" }}>No messages yet — say hello 👋</div>
          )}
          {messages?.map((m) => (
          ...
          {canSend ? (
            <form onSubmit={submit} style={{ display: "flex", gap: 10 }}>
              <input
                ...
                placeholder="Type a message... (no phone numbers, emails, or links — keep it on Projex.pk)"
                style={{ flex: 1, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", fontSize: "0.87rem", fontFamily: "'Inter', sans-serif", color: C.navy, outline: "none" }}
              />
      ...
      {/* sidebar */}
      <div style={{ border: `1px solid ${C.border}`, borderRadius: 16, background: "#fff", padding: 18, alignSelf: "start" }}>
        {yourRole && (
      ...
      {flagTarget && (
        <FlagModal message={flagTarget} onClose={() => setFlagTarget(null)} onFlagged={() => setToast("Message reported — our team will review it.")} />
      )}
...
function MessageBubble({ message, mine, onFlag }) {
  ...
  return (
    ...
    >
      {hov && (
        <button onClick={onFlag} title="Report message" style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.8rem", color: C.muted, order: mine ? -1 : 1 }}>🚩</button>
      )}
      <div style={{ maxWidth: "72%" }}>
        <div style={{
          background: mine ? C.navy : C.cream, color: mine ? "#fff" : C.navy,
          borderRadius: mine ? "14px 14px 3px 14px" : "14px 14px 14px 3px",
          padding: "9px 14px", fontSize: "0.86rem", lineHeight: 1.5, wordBreak: "break-word",
        }}>
          {message.content}
        </div>
        <div style={{ fontSize: "0.68rem", color: C.muted, marginTop: 3, textAlign: mine ? "right" : "left" }}>
...
function FlagModal({ message, onClose, onFlagged }) {
  ...
  return (
    <Modal onClose={loading ? () => {} : onClose} width={420}>
      ...
      <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="What felt inappropriate about this?" rows={3}
