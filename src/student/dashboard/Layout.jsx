...
/* ══════════════════════════════════════════
   SIDEBAR — collapsible + manually resizable
   Collapsed = 64px (icons only)
   Expanded  = user-draggable (default 220px)
══════════════════════════════════════════ */
export function Sidebar({ active, onNav, onLogout, collapsed, onToggle, width }) {
  const w = collapsed ? 64 : width;

  // safely parse user from localStorage
  let user = {};
  try { user = JSON.parse(localStorage.getItem("user") || "{}"); } catch {}
  const initials = (user.full_name || "ST").split(" ").slice(0,2).map(n=>n[0]?.toUpperCase()).join("") || "ST";
  ...
  return (
    <aside style={{
      ...
      <div style={{ padding:"18px 14px 16px", borderBottom:"1px solid rgba(255,255,255,0.07)", display:"flex", alignItems:"center", justifyContent:"space-between", minHeight:64 }}>
        ...
        <button onClick={onToggle} style={{ background:"rgba(255,255,255,0.06)", border:"none", borderRadius:6, width:26, height:26, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, color:"rgba(255,255,255,0.4)", transition:"all 0.15s" }}
        ...
        {collapsed ? (
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:"0.78rem", fontWeight:700, color:"#fff", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{user.full_name || "Student"}</div>
              <div style={{ fontSize:"0.66rem", color:"rgba(255,255,255,0.32)", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{user.university_name || ""}</div>
            </div>
            <LogOut size={13} color="rgba(255,255,255,0.25)" style={{ flexShrink:0, cursor:"pointer" }} onClick={onLogout}
