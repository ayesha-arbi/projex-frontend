...
// If not set, defaults to /api (works when frontend + backend on same origin)
...
async function submitProject(form) {
  ...
  // Guard against HTML error pages (404, 500, nginx errors)
...
function Banner({ children, type="info" }) {
  const map = { info:{bg:C.blueTint,border:"rgba(3,62,102,0.15)",color:C.blue,icon:<Info size={14}/>}, warning:{bg:"#fef3c7",border:"#fde68a",color:"#78350f",icon:<Lock size={14}/>}, error:{bg:"#fef2f2",border:"#fecaca",color:"#dc2626",icon:<AlertTriangle size={14}/>} };
...
/* ══════════════════════════════════════════
   MAIN EXPORT
══════════════════════════════════════════ */
export default function UploadProjectTab() {
  ...
  const handlePost = async () => {
    ...
    try { const r = await submitProject(form); setCreatedProject(r.project); setSubmitted(true); }
    catch (err) { setSubmitError(err.message); }
    finally { setSubmitting(false); }
  };

  const reset = () => { setSubmitted(false); setStep(1); setForm(EMPTY_FORM); setCreatedProject(null); setSubmitError(null); };
    ...
    return (
      <div style={{ padding:"32px", display:"flex", alignItems:"center", justifyContent:"center", minHeight:"55vh" }}>
        <div style={{ maxWidth:420, width:"100%", textAlign:"center", animation:"fadeUp 0.5s ease both" }}>
          ...
          <p style={{ color:C.muted, lineHeight:1.7, marginBottom:10, fontSize:"0.88rem" }}><strong style={{ color:C.text }}>{createdProject.title}</strong> is now live on Projex.pk.</p>
          <div style={{ background:C.blueTint, border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 16px", marginBottom:22, display:"inline-flex", alignItems:"center", gap:8, fontSize:"0.76rem", color:C.blue, fontWeight:600 }}>
            <CheckCircle size={13} color={C.greenDark} /> ID: {createdProject.project_id}
          </div>
          <p style={{ fontSize:"0.82rem", color:C.muted, lineHeight:1.7, marginBottom:24 }}>Full details stay locked until you approve each company request.</p>
          <Btn variant="primary" onClick={reset}><Plus size={13} /> Post Another</Btn>
        </div>
      </div>
    );
  }
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          {step<4
            ...
            : <Btn variant="green" size="sm" disabled={!form.poster||submitting} onClick={handlePost}>
                {submitting ? <><span style={{ width:13, height:13, border:"2px solid rgba(255,255,255,0.4)", borderTopColor:"#fff", borderRadius:"50%", display:"inline-block", animation:"spin 0.7s linear infinite" }}/> Posting…</> : <><Rocket size={13}/> Post Project</>}
              </Btn>
          }
        </div>
      </div>
