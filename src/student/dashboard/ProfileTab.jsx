...
function Btn({ children, variant = "primary", size = "sm", onClick, disabled = false, style: ex = {} }) {
  ...
  const v = {
    ...
    green:   { background:hov?"#b8e047":C.gold, color:C.navy, boxShadow:hov?"0 6px 18px rgba(176,141,87,0.3)":"none" },
  };
...
function CardHeader({ title, icon: Icon, accent = C.navy }) {
  return (
    <div style={{ padding:"13px 18px", borderBottom:`1px solid ${C.border}`, background:C.cream, display:"flex", alignItems:"center", gap:8 }}>
    ...
    <div style={{ display:"flex", alignItems:"center", padding:"9px 0", borderBottom:`1px solid ${C.border}` }}>
      ...
      {editing ? (
        <input defaultValue={value} onChange={e=>onChange?.(e.target.value)}
          style={{ flex:1, padding:"5px 8px", border:`1.5px solid ${C.border}`, borderRadius:7, fontSize:"0.83rem", fontFamily:"'Sora',sans-serif", color:C.text, outline:"none", background:C.white }}
  ...
  return (
    <div style={{ padding:"32px 48px 48px", width:"100%", boxSizing:"border-box", animation:"fadeUp 0.3s ease both" }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:24 }}>
        <div>
          <h2 style={{ fontSize:"1.1rem", fontWeight:800, color:C.navy, margin:0, fontFamily:"'Sora',sans-serif" }}>Profile</h2>
      ...
      {/* ── Saved toast ── */}
      {saved && (
        <div style={{ background:C.goldPale, border:`1px solid rgba(122,170,28,0.35)`, borderRadius:10, padding:"10px 16px", marginBottom:16, display:"flex", alignItems:"center", gap:8, fontSize:"0.82rem", color:C.goldDark, fontWeight:600, animation:"fadeUp 0.3s ease both" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:16 }}>
        ...
        <BentoCard style={{ gridColumn:"span 2", padding:"24px" }} accent={C.navy}>
          <div style={{ display:"flex", alignItems:"center", gap:20 }}>

            {/* Avatar */}
            <div style={{ position:"relative", flexShrink:0 }}>
              <div style={{ width:72, height:72, borderRadius:"50%", background:`linear-gradient(135deg,${C.navy},${C.navyMid})`, border:`3px solid ${C.gold}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.5rem", fontWeight:800, color:"#fff", letterSpacing:"-1px", boxShadow:`0 0 0 4px ${C.gold}30` }}>
                {initials}
              </div>
              {editing && (
                <button style={{ position:"absolute", bottom:0, right:0, width:24, height:24, borderRadius:"50%", background:C.navy, border:`2px solid ${C.white}`, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 2px 8px rgba(12,35,64,0.3)" }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              {editing ? (
                <input value={form.full_name} onChange={e=>set("full_name",e.target.value)}
                  style={{ fontSize:"1.2rem", fontWeight:800, color:C.navy, fontFamily:"'Sora',sans-serif", border:`1.5px solid ${C.border}`, borderRadius:8, padding:"6px 10px", outline:"none", width:"100%", marginBottom:8, background:C.white, boxSizing:"border-box" }}
                  onFocus={e=>e.target.style.borderColor=C.navy}
                  onBlur={e=>e.target.style.borderColor=C.border}
                />
              ) : (
                <h3 style={{ fontSize:"1.2rem", fontWeight:800, color:C.navy, margin:"0 0 5px", fontFamily:"'Sora',sans-serif" }}>
              ...
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                <Badge color={C.navy}><GraduationCap size={10}/>Student</Badge>
                {form.degree_program && <Badge color={C.navyMid}><Cpu size={10}/>{form.degree_program.replace("BS ","")}</Badge>}
        ...
        <BentoCard style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"24px 20px", textAlign:"center" }} accent={C.gold}>
          {/* Circular progress */}
          <div style={{ position:"relative", width:80, height:80, marginBottom:12 }}>
            ...
            <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
              <span style={{ fontSize:"1.1rem", fontWeight:800, color:C.navy, lineHeight:1, fontFamily:"'Sora',sans-serif" }}>{pct}%</span>
            </div>
          </div>
          ...
          {pct < 100 && !editing && (
            <button onClick={()=>setEditing(true)} style={{ marginTop:12, fontSize:"0.72rem", fontWeight:700, color:C.navy, background:C.navyTint, border:`1px solid ${C.border}`, borderRadius:6, padding:"5px 12px", cursor:"pointer", fontFamily:"'Sora',sans-serif" }}>
              Complete Profile
            </button>
          )}
        </BentoCard>
        ...
        {/* ── Academic Info (1 col) ── */}
        <BentoCard>
          <CardHeader title="Academic Info" icon={Building2} accent={C.navy}/>
        ...
        {/* ── Bio (1 col) ── */}
        <BentoCard>
          <CardHeader title="Bio" icon={User} accent={C.goldDark}/>
          <div style={{ padding:"14px 18px" }}>
            {editing ? (
              <div style={{ position:"relative" }}>
                <textarea rows={6} value={form.bio}
                  ...
                  style={{ width:"100%", boxSizing:"border-box", padding:"10px 12px", borderRadius:9, border:`1.5px solid ${C.border}`, fontSize:"0.84rem", fontFamily:"'Sora',sans-serif", color:C.text, background:C.white, outline:"none", resize:"none", lineHeight:1.65 }}
                  onFocus={e=>e.target.style.borderColor=C.navy}
                  onBlur={e=>e.target.style.borderColor=C.border}
                />
        ...
        {/* ── Skills (1 col) ── */}
        <BentoCard>
          <CardHeader title="Skills" icon={Award} accent={C.navyMid}/>
          <div style={{ padding:"14px 18px" }}>
            {editing ? (
              <div>
                <input value={form.skills} onChange={e=>set("skills",e.target.value)}
                  placeholder="e.g. Python, React, Machine Learning"
                  style={{ width:"100%", boxSizing:"border-box", padding:"9px 12px", borderRadius:9, border:`1.5px solid ${C.border}`, fontSize:"0.84rem", fontFamily:"'Sora',sans-serif", color:C.text, background:C.white, outline:"none", marginBottom:8 }}
                  onFocus={e=>e.target.style.borderColor=C.navy}
                  onBlur={e=>e.target.style.borderColor=C.border}
                />
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                {form.skills
                  ? form.skills.split(",").map(s=>s.trim()).filter(Boolean).map(sk=>(
                      <Badge key={sk} color={C.navyMid}><BookOpen size={9}/>{sk}</Badge>
                    ))
            ...
            <div>
              ...
              {editing ? (
                <input defaultValue={form.linkedin_url} onChange={e=>set("linkedin_url",e.target.value)}
                  ...
                  style={{ width:"100%", boxSizing:"border-box", padding:"9px 12px", borderRadius:9, border:`1.5px solid ${C.border}`, fontSize:"0.84rem", fontFamily:"'Sora',sans-serif", color:C.text, background:C.white, outline:"none" }}
                  onFocus={e=>e.target.style.borderColor=C.navy}
                  onBlur={e=>e.target.style.borderColor=C.border}
                />
              ) : (
                <p style={{ fontSize:"0.84rem", color:form.linkedin_url?C.navy:C.muted2, fontStyle:form.linkedin_url?"normal":"italic", margin:0 }}>
